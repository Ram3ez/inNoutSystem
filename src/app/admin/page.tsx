 ...
 * 2. Movement Enforcement: Administrative blocking of student outings.
 * 3. Institutional Calendar: Management of holidays and academic dates.
 * 4. Reporting Engine: Advanced CSV export with IST-boundary precision.
 * 5. Staff Orchestration: Assignment of Faculty and Caretakers to student groups.
 */
export default function AdminPortal() {
  ...
  const [isDownloadingReport, setIsDownloadingReport] = useState<string | null>(
    null,
  );
  ...
  // Outings Tab State
  const [outingsSubTab, setOutingsSubTab] = useState<"current" | "archived">(
    "current",
  );
  ...
  const [outingsDateType, setOutingsDateType] = useState<
    "out_time" | "in_time"
  >("out_time");
  ...
  // Leaves Tab State
  const [leavesSubTab, setLeavesSubTab] = useState<"current" | "archived">(
    "current",
  );
  const [leavesPage, setLeavesPage] = useState(1);
  const [leavesRollQuery, setLeavesRollQuery] = useState("");
  const [leavesDateQuery, setLeavesDateQuery] = useState("");
  const [leavesDateType, setLeavesDateType] = useState<
    "proposed_exit_date" | "proposed_in_date"
  >("proposed_exit_date");
  ...
  const [holidayType, setHolidayType] = useState<"GAZETTED" | "RESTRICTED">(
    "GAZETTED",
  );
  ...
  const staffSuggestions = Array.from(
    new Map(
      [...facultyMembers, ...caretakerMembers].map((m) => [
        m.userEmail,
        { email: m.userEmail, name: m.userName },
      ]),
    ).values(),
  ).filter((u) => u.email && !isStudentEmail(u.email));
  ...
  const handleSaveHoliday = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!holidayName.trim()) {
      setNotification({ message: "Holiday Name is required", type: "error" });
  ...
  const handleUpdateStaff = async (
    ...
    try {
      ...
      if (collId === COLLECTIONS.CARETAKER) {
        setCaretakerAssignments((prev) =>
          prev.map((d) => (d.$id === docId ? { ...d, email } : d)),
        );
      } else {
  ...
  const fetchOutingsData = async () => {
    ...
    try {
      ...
      if (outingsRollQuery.trim()) {
        queries.push(
          Query.contains("roll_no", outingsRollQuery.trim().toUpperCase()),
        );
      }

      if (
        outingsDateQuery &&
        outingsDateQuery.length === 10 &&
        /^20\d{2}-\d{2}-\d{2}$/.test(outingsDateQuery)
      ) {
        ...
        const testFormats = [
          ...
          {
            start: utcStart.replace(".000Z", "Z"),
            end: utcEnd.replace(".999Z", "Z"),
          },
          {
            start: utcStart.replace("T", " ").replace(".000Z", ""),
            end: utcEnd.replace("T", " ").replace(".999Z", ""),
          },
          {
            start: utcStart.replace(".000Z", ""),
            end: utcEnd.replace(".999Z", ""),
          },
        ];
  ...
  const fetchLeavesData = async () => {
    ...
    try {
      ...
      if (leavesRollQuery.trim()) {
        queries.push(
          Query.contains("roll_no", leavesRollQuery.trim().toUpperCase()),
        );
      }

      if (
        leavesDateQuery &&
        leavesDateQuery.length === 10 &&
        /^20\d{2}-\d{2}-\d{2}$/.test(leavesDateQuery)
      ) {
  ...
  const handleDownloadReport = async (
    ...
    try {
      ...
      if (duration === "day") {
        ...
        endDate = new Date(
          `${selectedDate}-${String(lastDay).padStart(2, "0")}T23:59:59.999${tz}`,
        );
      }
      ...
      reportData.sort(
        (a, b) =>
          new Date(b[dateAttr]).getTime() - new Date(a[dateAttr]).getTime(),
      );
      ...
      const rows = reportData.map((item) => {
        ...
        return [roll, name, formatCSVDate(exit), formatCSVDate(entry)].join(
          ",",
        );
      });
      ...
      link.setAttribute(
        "download",
        `${type}_report_${duration}_${filenameDate}.csv`,
      );
  ...
  const handleBlockStudent = async (
    ...
    try {
      ...
      setStudents((prev) =>
        prev.map((s) =>
          s.$id === studentId
            ? { ...s, outing_blocked_until: until || undefined }
            : s,
        ),
      );
      ...
      <AnimatePresence>
        {notification && (
          <motion.div
            ...
            className={`fixed top-24 right-6 z-[200] p-4 rounded-2xl border backdrop-blur-xl shadow-2xl flex items-center gap-3 min-w-[320px] max-w-md ${
        ...
        <header className="mb-12 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="flex items-center space-x-4 shrink-0">
            ...
            <div className="text-left whitespace-nowrap">
              <p className="text-secondary font-black tracking-[0.3em] text-[9px] sm:text-[10px] uppercase mb-1">
                Administrative Access
              </p>
              <h1 className="text-2xl sm:text-3xl font-black text-primary tracking-tight uppercase flex items-center justify-start space-x-3 italic whitespace-nowrap">
          ...
          {activeTab === "students" ? (
            ...
            >
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-center space-x-0 md:space-x-4 w-full md:w-1/3 text-center md:text-left space-y-4 md:space-y-0">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-1">
                          <p className="text-primary/30 text-[10px] font-bold uppercase tracking-wider">
                            {student.$id}
                          </p>
                          {student.course && (
                            <>
                              <span className="text-primary/10 text-[8px]">
                                •
                              </span>
                              ...
                              <p className="text-primary/40 font-bold text-[9px] uppercase tracking-widest">
                                {student.department}
                              </p>
                            </>
                          )}
                    ...
                    <div className="w-full md:w-1/4 flex flex-col items-center space-y-2">
                      {student.ghostface_registered ? (
                        <div className="flex items-center space-x-2 text-secondary bg-secondary/10 px-3 py-1 rounded-full border border-secondary/20">
                          ...
                          <button
                            onClick={() =>
                              handleDeleteFace(student.$id, "ghostface")
                            }
                      ...
                      {student.edgeface_registered ? (
                        <div className="flex items-center space-x-2 text-indigo-400 bg-indigo-500/10 px-3 py-1 rounded-full border border-indigo-500/20">
                          ...
                          <button
                            onClick={() =>
                              handleDeleteFace(student.$id, "edgeface")
                            }
                      ...
                      <button
                        onClick={() =>
                          setBlockConfig({
                            ...
                            blockedUntil: student.outing_blocked_until
                              ? new Date(student.outing_blocked_until)
                                  .toISOString()
                                  .split("T")[0]
              ...
              {filteredStudents.length === 0 && (
                <div className="py-20 text-center space-y-4">
                  ...
                  <p className="text-primary/20 text-xs font-bold uppercase tracking-[0.2em]">
                    No students found
                  </p>
                </div>
              )}
            ...
            >
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 bg-surface border border-primary/5 p-6 rounded-[2.5rem] shadow-xl shadow-primary/5">
                <div className="flex bg-primary/5 p-1 rounded-2xl border border-primary/5 w-full sm:w-auto self-start">
                  <button
                    onClick={() => {
                      setOutingsSubTab("current");
                      setOutingsPage(1);
                    }}
                    className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all text-center ${outingsSubTab === "current" ? "bg-primary text-background shadow-lg shadow-primary/20" : "text-primary/60 hover:text-primary"}`}
                  >
                    Current Outings
                  </button>
                  <button
                    onClick={() => {
                      setOutingsSubTab("archived");
                      setOutingsPage(1);
                    }}
                    className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all text-center ${outingsSubTab === "archived" ? "bg-primary text-background shadow-lg shadow-primary/20" : "text-primary/60 hover:text-primary"}`}
                  >
                    Archived Outings
                  </button>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                  <div className="relative group w-full sm:w-56">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setOutingsRollQuery(e.target.value);
                        setOutingsPage(1);
                      }}
                  ...
                  <div className="relative w-full sm:w-40 z-30">
                    <button
                      onClick={() =>
                        setIsOutingsDateTypeOpen(!isOutingsDateTypeOpen)
                      }
                    ...
                    >
                      <span className="truncate">
                        {outingsDateType === "out_time"
                          ? "Out Time"
                          : "In Time"}
                      </span>
                      <ChevronDown
                        size={14}
                        className={`text-primary/40 transition-transform duration-200 shrink-0 ${isOutingsDateTypeOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    <AnimatePresence>
                      {isOutingsDateTypeOpen && (
                        <>
                          <div
                            ...
                            onClick={() => setIsOutingsDateTypeOpen(false)}
                          />
                          <motion.div
                          ...
                          >
                            <button
                              onClick={() => {
                                setOutingsDateType("out_time");
                                setIsOutingsDateTypeOpen(false);
                                setOutingsPage(1);
                              }}
                  ...
                  <div className="relative group w-full sm:w-48">
                    <input
                      ...
                      onChange={(e) => {
                        setOutingsDateQuery(e.target.value);
                        setOutingsPage(1);
                      }}
                ...
                <div className="space-y-4">
                  {outings.map((outing) => (
                    ...
                    >
                        ...
                        <div>
                          ...
                          {outing.in_time && (
                            <p className="text-secondary/70 text-[10px] font-black tracking-widest uppercase mt-0.5">
                              In Time:{" "}
                              {new Date(outing.in_time).toLocaleString(
                                "en-IN",
                                { timeZone: "Asia/Kolkata" },
                              )}
                            </p>
                      ...
                      <div>
                        {outing.in_time ? (
                          <span className="text-[9px] bg-primary/5 text-primary/40 px-3 py-1 rounded-full border border-primary/10 font-black tracking-widest uppercase">
                            Returned
                          </span>
                        ) : (
                          <span className="text-[9px] bg-secondary/10 text-secondary px-3 py-1 rounded-full border border-secondary/20 font-black tracking-widest uppercase animate-pulse">
                            Active Out
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center justify-between pt-4">
                    <button
                      ...
                      onClick={() =>
                        setOutingsPage((prev) => Math.max(1, prev - 1))
                      }
            ...
            >
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 bg-surface border border-primary/5 p-6 rounded-[2.5rem] shadow-xl shadow-primary/5">
                <div className="flex bg-primary/5 p-1 rounded-2xl border border-primary/5 w-full sm:w-auto self-start">
                  <button
                    onClick={() => {
                      setLeavesSubTab("current");
                      setLeavesPage(1);
                    }}
                    className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all text-center ${leavesSubTab === "current" ? "bg-primary text-background shadow-lg shadow-primary/20" : "text-primary/60 hover:text-primary"}`}
                  >
                    Current Leaves
                  </button>
                  <button
                    onClick={() => {
                      setLeavesSubTab("archived");
                      setLeavesPage(1);
                    }}
                    className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all text-center ${leavesSubTab === "archived" ? "bg-primary text-background shadow-lg shadow-primary/20" : "text-primary/60 hover:text-primary"}`}
                  >
                    Archived Leaves
                  </button>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full xl:w-auto">
                  <div className="relative group w-full sm:w-56">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setLeavesRollQuery(e.target.value);
                        setLeavesPage(1);
                      }}
                      className="w-full bg-surface border border-primary/10 rounded-2xl h-11 pl-11 pr-4 text-primary text-xs focus:outline-none focus:border-secondary/50 transition-all uppercase placeholder:text-primary/20"
                    />
                  ...
                  <div className="relative w-full sm:w-40 z-30">
                    <button
                      onClick={() =>
                        setIsLeavesDateTypeOpen(!isLeavesDateTypeOpen)
                      }
                      className="w-full bg-surface border border-primary/10 rounded-2xl h-11 px-4 text-primary text-xs font-bold tracking-wider uppercase transition-all flex items-center justify-between hover:border-primary/30 select-none cursor-pointer"
                    >
                      <span className="truncate">
                        {leavesDateType === "proposed_exit_date"
                          ? "Departure"
                          : "Return"}
                      </span>
                      <ChevronDown
                        size={14}
                        className={`text-primary/40 transition-transform duration-200 shrink-0 ${isLeavesDateTypeOpen ? "rotate-180" : ""}`}
                      />
                    </button>
                    <AnimatePresence>
                      {isLeavesDateTypeOpen && (
                        <>
                          <div
                            className="fixed inset-0 z-10"
                            onClick={() => setIsLeavesDateTypeOpen(false)}
                          />
                          <motion.div
                            initial={{ opacity: 0, y: -4 }}
                            animate={{ opacity: 1, y: 0 }}
                          ...
                          >
                            <button
                              onClick={() => {
                                setLeavesDateType("proposed_exit_date");
                                setIsLeavesDateTypeOpen(false);
                                setLeavesPage(1);
                              }}
                            ...
                            <button
                              onClick={() => {
                                setLeavesDateType("proposed_in_date");
                                setIsLeavesDateTypeOpen(false);
                                setLeavesPage(1);
                              }}
                  ...
                  <div className="relative group w-full sm:w-48">
                    <input
                      ...
                      onChange={(e) => {
                        setLeavesDateQuery(e.target.value);
                        setLeavesPage(1);
                      }}
                      className="w-full bg-surface border border-primary/10 rounded-2xl h-11 px-4 text-primary text-xs focus:outline-none focus:border-secondary/50 transition-all uppercase placeholder:text-primary/20"
                    />
                ...
                <div className="space-y-4">
                      ...
                      <div className="flex flex-col md:flex-row items-center gap-4 text-center md:text-left w-full">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                            ...
                            <span
                              className={`text-[8px] sm:text-[9px] px-2.5 py-0.5 rounded-full border font-black uppercase tracking-wider ${leave.status === "approved" ? "bg-green-500/10 text-green-600 border-green-500/10" : "bg-orange-500/10 text-orange-600 border-orange-500/10"}`}
                            >
                              {leave.status}
                            </span>
                  ...
                  <div className="flex items-center justify-between pt-4">
                    <button
                      ...
                      onClick={() =>
                        setLeavesPage((prev) => Math.max(1, prev - 1))
                      }
                      className="px-5 py-2.5 bg-surface border border-primary/5 hover:border-primary/20 text-primary/60 hover:text-primary rounded-xl font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-30 disabled:pointer-events-none"
                    >
                      Previous
            ...
            >
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 bg-surface border border-primary/5 p-6 rounded-[2.5rem] shadow-xl shadow-primary/5">
                ...
                >
                  <RefreshCw
                    className={`h-4 w-4 ${isHolidaysLoading ? "animate-spin" : ""}`}
                  />
                  <span>Sync</span>
                </button>
              </div>
                      ...
                      return days.map((day, idx) => {
                        if (!day)
                          return (
                            <div
                              key={`empty-${idx}`}
                              className="aspect-square sm:aspect-auto sm:min-h-[110px] w-full bg-transparent rounded-2xl select-none"
                            />
                          );
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`text-xs font-black self-end ${isToday ? "text-secondary font-black" : "text-primary/60"}`}
                            >
                              {day.getDate()}
                            </span>
                ...
                <div className="bg-surface border border-primary/5 rounded-[2.5rem] p-6 shadow-xl shadow-primary/5 flex flex-col justify-between">
                  <div className="space-y-6">
                    <div className="space-y-1">
                      <h3 className="text-sm font-black tracking-tight text-primary uppercase italic">
                        Legend & Selected Month
                      </h3>
                      <p className="text-primary/40 text-[10px] font-bold uppercase tracking-wider">
                        Configure specific holidays directly
                      </p>
                    </div>
                      ...
                      <div className="flex items-center gap-3 bg-primary/5 p-3 rounded-2xl border border-primary/5">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-primary/40">
                            Mandatory institutional leaves
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                      <h4 className="text-[10px] font-black tracking-widest text-primary/30 uppercase">
                        Month Overview
                      </h4>
                      {holidays
                        .filter((h) => {
                        ...
                        .map((h) => (
                          ...
                          >
                            <div className="flex flex-col max-w-[70%]">
                              <span className="text-[9px] font-black text-secondary tracking-widest">
                                {h.date}
                              </span>
                              ...
                              {h.description && (
                                <span className="text-[9px] text-primary/40 truncate mt-0.5">
                                  {h.description}
                                </span>
                              )}
                            </div>
                      ...
                      {holidays.filter((h) => {
                        ...
                        <p className="text-[10px] font-bold text-primary/20 py-2 text-center uppercase tracking-widest">
                          No holidays listed
                        </p>
                      )}
                    </div>
                      ...
                      <div>
                        ...
                        >
                          <div className="space-y-2">
                            <label className="text-[10px] font-black tracking-widest uppercase text-primary/40 ml-2">
                              Type
                            </label>
              ...
              <section className="max-w-5xl mx-auto bg-surface border border-primary/5 p-8 rounded-[3rem] space-y-8 shadow-xl shadow-primary/5">
                <div className="text-center space-y-2">
                  <h2 className="text-2xl font-black text-primary uppercase tracking-tight italic">
                    Add to {activeTab === "faculty" ? "Faculty" : "Caretaker"}{" "}
                    Team
                  </h2>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2 relative">
                      ...
                      <input
                        ...
                        onBlur={() =>
                          setTimeout(() => setShowInviteSuggestions(false), 200)
                        }
                        className="w-full bg-primary/5 border border-primary/10 rounded-2xl h-14 px-6 text-sm font-bold text-primary focus:border-secondary transition-all"
                      />
                  ...
                  >
                    ...
                    <span>
                      Invite to{" "}
                      {activeTab === "faculty" ? "Faculty" : "Caretaker"}
                    </span>
                  </button>
                </div>
              </section>
              ...
              <section className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {(activeTab === "faculty"
                    ...
                    <div
                      key={member.$id}
                      className="bg-surface border border-primary/5 p-6 rounded-[2.5rem] flex flex-col justify-between group hover:bg-surface/50 transition-all gap-4"
                    >
                      <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          {member.joined ? (
                            <span className="text-[8px] bg-green-500/10 text-green-600 px-2 py-1 rounded-full border border-green-500/10 font-black uppercase tracking-tighter">
                              Active
                            </span>
                          ) : (
                            <span className="text-[8px] bg-orange-500/10 text-orange-600 px-2 py-1 rounded-full border border-orange-500/10 font-black uppercase tracking-tighter">
                              Invited
                            </span>
                          )}
                        </div>
                ...
                {(activeTab === "faculty" ? facultyMembers : caretakerMembers)
                  .length === 0 && (
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        type={
                          exportConfig.duration === "month" ? "month" : "date"
                        }
                    ...
                    {exportConfig.duration === "week" && (
                      <p className="text-[9px] font-bold text-primary/30 uppercase tracking-wider mt-2 ml-2 italic">
                        * Report will cover 7 days starting from the selected
                        date
                      </p>
                    )}
                  </div>
                  ...
                  <button
                    disabled={
                      !exportConfig.selectedDate || isDownloadingReport !== null
                    }
                    onClick={() =>
                      handleDownloadReport(
                        exportConfig.type!,
                        exportConfig.duration!,
                        exportConfig.selectedDate,
                      )
                    }
              ...
              >
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-primary uppercase tracking-tight truncate max-w-[200px]">
                        {blockConfig.studentName}
                      </h3>
                      <p className="text-[10px] font-bold text-primary/40 uppercase tracking-widest">
                        Restrict Outing
                      </p>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <p className="text-[9px] font-bold text-primary/30 uppercase tracking-wider mt-2 ml-2 italic">
                      * Student will be unable to take outings until 11:59 PM of
                      this date
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      onClick={() => {
                        const untilDate = new Date(
                          `${blockConfig.blockedUntil}T23:59:59+05:30`,
                        ).toISOString();
                    ...
                    <button
                      onClick={() =>
                        handleBlockStudent(blockConfig.studentId!, null)
                      }
